...
export class XcodeDebugger extends EventEmitter {
  private lldbProcess: ChildProcess | null = null;
  private isAttached: boolean = false;
  private currentProcess: { pid: number; name: string } | null = null;
  ...
  async attachToProcess(pid?: number, processName?: string): Promise<{ success: boolean; message: string }> {
    try {
      if (this.isAttached) {
  ...
  async setBreakpoint(
    ...
    symbol?: string,
    condition?: string
  ): Promise<{ success: boolean; breakpointId?: number; message: string }> {
  ...
  async continueExecution(): Promise<{ success: boolean; message: string }> {
    return this.runLLDBCommand('continue');
  }

  async stepOver(): Promise<{ success: boolean; message: string }> {
    return this.runLLDBCommand('next');
  }

  async stepInto(): Promise<{ success: boolean; message: string }> {
    return this.runLLDBCommand('step');
  }

  async stepOut(): Promise<{ success: boolean; message: string }> {
    return this.runLLDBCommand('finish');
  }

  async inspectVariable(expression: string): Promise<{ success: boolean; value?: string; message: string }> {
    try {
  ...
  async getStackTrace(): Promise<{ success: boolean; frames?: Array<{
    index: number;
    function: string;
    file: string;
    line: number;
  ...
  private parseStackTrace(output: string): Array<{
    index: number;
    function: string;
    file: string;
    line: number;
  ...
  async listBreakpoints(): Promise<{ success: boolean; breakpoints: Array<{
    id: number;
    file?: string;
    line?: number;
    symbol?: string;
  ...
  async deleteBreakpoint(breakpointId: number): Promise<{ success: boolean; message: string }> {
    try {
  ...
  async runLLDBCommand(command: string): Promise<{ success: boolean; output: string; message: string }> {
    if (!this.isAttached || !this.lldbProcess) {
  ...
  async detach(): Promise<{ success: boolean; message: string }> {
    try {
      if (!this.isAttached) {
