...
class FactoryForgeDebugMCPServer {
  ...
  private debugger: XcodeDebugger;
      ...
      try {
        switch (name) {
          case 'attach_to_process':
            const attachResult = await this.debugger.attachToProcess(
              args.processId as number | undefined,
              args.processName as string | undefined
            );
            return {
          ...
          case 'set_breakpoint':
            const breakpointResult = await this.debugger.setBreakpoint(
          ...
          case 'inspect_variable':
            const inspectResult = await this.debugger.inspectVariable(args.expression as string);
            return {
          ...
          case 'delete_breakpoint':
            const deleteResult = await this.debugger.deleteBreakpoint(args.breakpointId as number);
            return {
              content: [{ type: 'text', text: JSON.stringify(deleteResult, null, 2) }],
            };

          case 'run_lldb_command':
            const commandResult = await this.debugger.runLLDBCommand(args.command as string);
            return {
